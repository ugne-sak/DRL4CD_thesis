import numpy as onp
from functions.synthetic.utils import draw_rff_params
from functions.synthetic.abstract import NoiseModel
...
def init_noise_dist(*, rng, dim, dist, noise_scale, noise_scale_heteroscedastic):
    """
        dist: distribution for the noise, e.g. synthetic.distributions.Gaussian: ε ~ N(0, sigma^2)
        noise_scale: sigma_j is different for every noise variable ε_j so we define a distribution from which to sample sigma_j values, e.g. synthetic.distributions.Uniform
    """
